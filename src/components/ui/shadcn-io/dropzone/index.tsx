...
export const DropzoneEmptyState = ({ children, className }: DropzoneEmptyStateProps) => {
  ...
  //Overwrite caption
  caption = 'Accepts .epub and .kepub files';

  //Change files to books
  return (
    <div className={cn('flex flex-col items-center justify-center', className)}>
      <div className="flex size-8 items-center justify-center rounded-md bg-muted text-muted-foreground">
        <UploadIcon size={16} />
      </div>
      <p className="my-2 w-full truncate text-wrap font-medium text-sm">Upload {maxFiles === 1 ? 'a book' : 'books'}</p>
